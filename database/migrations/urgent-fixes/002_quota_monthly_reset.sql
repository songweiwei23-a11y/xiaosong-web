...
-- 方案1: 添加 last_reset_at 字段，在使用时检查并重置
-- 优点: 简单可靠，无需定时任务
-- 缺点: 需要修改代码逻辑
...
-- 4. 创建增加使用次数的函数（替代原有的 increment_quota_used）
...
-- 在代码中调用方式：
-- 1. 获取已使用次数（会自动重置）：
--    SELECT get_user_quota_used('user-uuid', 'script');
--
-- 2. 增加使用次数（会自动重置）：
--    SELECT increment_quota_used_v2('user-uuid', 'script');
--
-- 3. 管理员手动重置所有用户（紧急情况）：
--    SELECT admin_reset_all_quotas();

-- ============================================
-- 修复完成
-- ============================================

RAISE NOTICE '✓ 配额月度重置逻辑已实现';
RAISE NOTICE '✓ 关键函数: check_and_reset_monthly_quota, get_user_quota_used, increment_quota_used_v2';
