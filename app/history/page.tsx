...
export default function HistoryPage() {
  ...
  return (
    <div className="min-h-screen bg-muted dark:bg-muted">
      {/* Header */}
      <div className="glass-panel border-b border-border sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <Link 
              href="/dashboard" 
              className="text-sm text-primary hover:text-primary dark:text-primary dark:hover:text-primary transition-colors"
            >
              返回工作台
            </Link>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="glass-panel rounded-lg border border-border p-6">
        ...
        <div className="glass-panel rounded-lg border border-border overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-lg font-medium text-foreground mb-2">暂无生成记录</p>
              <p className="text-sm text-muted-foreground">开始使用工作台生成内容吧</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-200 dark:divide-slate-800">
              {history.map((item) => (
                <div
                  key={item.id}
                  className="p-6 hover:bg-foreground/[0.06] dark:hover:bg-muted/50 transition-colors"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-primary/15 dark:bg-blue-900/30 text-primary dark:text-primary">
                          {item.task_type || '未知类型'}
                        </span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button
                        ...
                        className="p-2 text-destructive hover:bg-destructive/10 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                        title="删除"
                        type="button"
                      >
